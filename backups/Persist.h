...
int System_Init() //系统初始化函数，创建系统所必需的文件夹
{
    int i;
...
int Client_Init(char *user_name) //客户端初始化函数，创建客户端所必需的的文件夹
{ 
    char path_1[40]="./";
    char path_2[40]="./";
...
int Service_Message_Save(char *name,char *file,message_node_t *data) //用户私聊信息写入函数，1成功0失败
{
    char path[50];
    int rtn=0;
	FILE *fp;
